...
import { AppState, ColorAlias, GlobalSettings, LoosePartsInventory, MyColorsSettings, UserInventory } from '../models/models';
...
export class StorageService {
    ...
    private migrateState(state: AppState): AppState {
        ...
        // Ensure My Colors settings exist with defaults
        const myColorsSettings: MyColorsSettings = state.myColorsSettings || {
            enabledColorIds: [],
            colorAliases: [],
            showHiddenColors: true,
            applyToSets: false
    ...
    /**
     * Update My Colors settings
     */
    updateMyColorsSettings(settings: Partial<MyColorsSettings>): void {
        const currentState = this.appStateSubject.getValue();
        const updatedState = {
            ...currentState,
            myColorsSettings: {
                ...currentState.myColorsSettings || {
                    enabledColorIds: [],
                    colorAliases: [],
                    showHiddenColors: true,
                    applyToSets: false
    ...
    /**
     * Get My Colors settings
     */
    getMyColorsSettings(): MyColorsSettings {
        const currentState = this.appStateSubject.getValue();
        return currentState.myColorsSettings || {
            enabledColorIds: [],
            colorAliases: [],
            showHiddenColors: true,
            applyToSets: false
    ...
    /**
     * Add or update a color alias
     */
    addOrUpdateColorAlias(alias: ColorAlias): void {
        const currentState = this.appStateSubject.getValue();
        const myColorsSettings = this.getMyColorsSettings();

        // Check if any of the colors in this alias are already in another alias
        const conflictingAlias = myColorsSettings.colorAliases.find(existing =>
            existing.id !== alias.id &&
            alias.colorIds.some((colorId: number) => existing.colorIds.includes(colorId))
        );

        if (conflictingAlias) {
            throw new Error(`One or more colors are already in the alias "${conflictingAlias.name}"`);
        }

        // Remove existing alias with same ID if updating
        const updatedAliases = myColorsSettings.colorAliases.filter(a => a.id !== alias.id);
        updatedAliases.push(alias);

        this.updateMyColorsSettings({
            colorAliases: updatedAliases
        });
    }

    /**
     * Remove a color alias
     */
    removeColorAlias(aliasId: string): void {
        const myColorsSettings = this.getMyColorsSettings();
        const updatedAliases = myColorsSettings.colorAliases.filter(a => a.id !== aliasId);
    ...
    /**
     * Check if a color is in any alias
     */
    isColorInAlias(colorId: number): ColorAlias | null {
        const myColorsSettings = this.getMyColorsSettings();
        return myColorsSettings.colorAliases.find(alias =>
