...
export class IndexedDBService {
    ...
    private readonly DB_VERSION = 7; // Updated for image cache
    private readonly STATE_KEY = 'brickInventoryAppState';
    private readonly CSV_CACHE_EXPIRY_HOURS = 720;
    private readonly IMAGE_CACHE_EXPIRY_DAYS = 30; // 30 days for images
            ...
            const initializeDB = async () => {
                try {
                    ...
                    db.version(4).stores({
                        ...
                        csv_colors: 'id, name, rgb, is_trans, num_parts, num_sets',
                    ...
                    // Version 6 schema (with color popularity data)
                    db.version(6).stores({
                    ...
                    // Version 7 schema (current version with image cache)
                    db.version(this.DB_VERSION).stores({
                        ...
                        // New image cache table
                        image_cache: 'url, timestamp, size'
                    }).upgrade(async (trans) => {
                        // Only upgrade message, no data clearing needed for adding image cache
                        console.log('Upgrading database to version 7: adding image cache table...');
                    });
    ...
    /**
     * Public method to manually clear stuck population flag
     */
    async clearStuckPopulationFlag(): Promise<void> {
        console.log('🔧 Manually clearing population flag...');
        await this.clearPopulationFlag();
        console.log('✅ Population flag cleared');
    }
    ...
    private async isPopulationInProgress(): Promise<boolean> {
        try {
            ...
            if (result && result.inProgress) {
                // Check if the flag is stale (older than 5 minutes - reduced from 30)
                const flagAge = Date.now() - (result.timestamp || 0);
                const fiveMinutes = 5 * 60 * 1000;

                if (flagAge > fiveMinutes) {
                    console.warn('🧹 Found stale population flag (older than 5 minutes), clearing it...');
                    console.warn(`Flag was set ${Math.round(flagAge / (60 * 1000))} minutes ago`);
                    // Clear the stale flag immediately
                    await this.clearPopulationFlag();
                    return false;
                } else {
                    ...
                    // If it's been more than 2 minutes, it's probably stuck - clear it
                    if (flagAge > 2 * 60 * 1000) {
                        console.warn('🔧 Population seems stuck after 2 minutes, clearing flag...');
                        await this.clearPopulationFlag();
                        return false;
                    }
     ...
     */
    async saveImageToCache(url: string, blob: Blob, contentType: string, etag?: string): Promise<void> {
        this.throwIfDisabled();
        ...
        try {
            ...
            await db.image_cache.put(imageData);
        } catch (error) {
     ...
     */
    async getImageFromCache(url: string): Promise<{ blob: Blob; contentType: string; etag?: string } | null> {
        this.throwIfDisabled();
        ...
        try {
            ...
            const maxAge = this.IMAGE_CACHE_EXPIRY_DAYS * 24 * 60 * 60 * 1000;
     ...
     */
    async hasImageInCache(url: string): Promise<boolean> {
        this.throwIfDisabled();
     ...
     */
    async deleteImageFromCache(url: string): Promise<void> {
        this.throwIfDisabled();
    ...
    async cleanupExpiredImages(): Promise<number> {
        ...
        try {
            ...
            for (const image of images) {
                ...
                if (age > maxAge) {
                    expiredUrls.push(image.url);
                }
            }

            if (expiredUrls.length > 0) {
                await db.image_cache.bulkDelete(expiredUrls);
            }
    ...
    /**
     * Get URLs that need to be cached
     */
    async getUncachedImageUrls(urls: string[]): Promise<string[]> {
        this.throwIfDisabled();
        ...
        try {
            ...
            for (const url of urls) {
                const hasImage = await this.hasImageInCache(url);
                if (!hasImage) {
            ...
            console.error('Error checking uncached image URLs:', error);
            return urls; // Return all URLs as uncached in case of error
        }
    }
}
