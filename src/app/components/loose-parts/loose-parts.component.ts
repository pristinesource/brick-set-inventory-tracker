...
import { Subject, combineLatest, debounceTime, distinctUntilChanged, map, takeUntil } from 'rxjs';
...
export class LoosePartsComponent implements OnInit, OnDestroy {
    ...
    // Navigation State
    navigationStep: 'sections' | 'categories' | 'parts' | 'search' = 'sections';
    ...
    ngOnInit(): void {
        ...
        this.partMappingService.initialize().catch(err => {
        ...
        this.searchSubject$.pipe(
            ...
            if (this.navigationStep === 'search' || this.navigationStep === 'parts') {
                this.currentPage = 1;
    ...
    createNewInventory(): void {
        ...
        this.switchInventory(newInventory.id);
    }
    ...
    viewCollectionInventory(): void {
        if (this.activeInventory) {
            this.router.navigate(['/collection-inventory', this.activeInventory.id]);
        }
    }
    ...
    onImageError(event: any, partNum?: string, colorId?: number): void {
        const img = event.target as HTMLImageElement;

        if (partNum && colorId && this.partColorsMap && this.elementsMap) {
            // Use the enhanced retry handler
    ...
    filterAndSortParts(): void {
        ...
        if (this.navigationStep === 'search') {
            // In search mode, search across all parts
        ...
        } else if (this.navigationStep !== 'parts' || !this.selectedSectionId || !this.selectedCategoryId) {
        ...
        // Start with all parts and filter appropriately
        let parts: PartWithDetails[] = this.allParts
            .filter(part => {
                // In search mode, include all parts; otherwise filter by category
                return this.navigationStep === 'search' || part.part_cat_id === this.selectedCategoryId;
            })
            .map(part => {
        ...
        if (this.searchTerm.trim()) {
            ...
            parts = parts.filter(part => {
                ...
                return searchWords.some(searchWord => {
                    // Check part name
                    if (part.name.toLowerCase().includes(searchWord)) {
                        return true;
                    }

                    // Check part number directly
                    if (part.part_num.toLowerCase().includes(searchWord)) {
                        return true;
                    }

                    // Check all possible part numbers (including BrickLink variants)
                    const allNumbers = this.partMappingService.getAllPartNumbers(part.part_num);
                    // Debug: Log first few part mappings
                    if (part.part_num === '70720') {
                        console.log('Part 70720 maps to:', allNumbers);
                    }
    ...
    performSearch(): void {
        ...
        this.navigationStep = 'search';
        this.selectedSectionId = null;
        this.selectedCategoryId = null;
        this.currentPage = 1;

        // Perform the search across all parts
        this.filterAndSortParts();

        // Update search results count
        this.searchResultsCount = this.filteredParts.length;
    }
    ...
    updatePartQuantity(part: PartWithDetails, newQuantity: number): void {
        ...
        if (newQuantity === 0 && existingEntry) {
            ...
            // Default to color tracking if the setting is enabled and this is a new entry
            const shouldTrackByColor = existingEntry?.trackByColor ||
                (this.globalSettings.alwaysTrackLoosePartsByColor && !existingEntry);

            const updatedEntry: LoosePartEntry = {
                trackByColor: shouldTrackByColor,
                totalQuantity: shouldTrackByColor ? undefined : newQuantity,
                colorQuantities: shouldTrackByColor ? (existingEntry?.colorQuantities || {}) : undefined,
                lastUpdated: Date.now()
            };

            // If tracking by color, open the color modal instead
            if (shouldTrackByColor && !existingEntry) {
                // New part with color tracking enabled - open color modal
                this.openColorTrackingModal(part);
                return;
            } else if (updatedEntry.trackByColor && updatedEntry.colorQuantities) {
                // Don't update if already in color tracking mode - use color modal instead
                return;
            }
    ...
    toggleColorTracking(part: PartWithDetails): void {
        ...
        // If always track by color is enabled and we're currently tracking by color, don't allow switching
        if (this.globalSettings.alwaysTrackLoosePartsByColor && currentlyTrackingByColor) {
            return;
        }
