...
export class InventoryDetailComponent implements OnInit, OnDestroy {
    ...
    onOverlayImageError(event: Event): void {
        ...
        // Try to extract part information from the overlay context
        ...
        if (!partNum && this.overlayImageAlt) {
            const allParts = [...this.parts, ...this.spareParts];
            const matchingPart = allParts.find(part =>
                part.part.name === this.overlayImageAlt ||
                this.overlayImageUrl === part.imageUrl
            );
            if (matchingPart) {
                partNum = matchingPart.part.part_num;
                colorId = matchingPart.color.id;
            }
        }
