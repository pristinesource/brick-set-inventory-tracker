...
export interface PartCategoryToSection {
    id: number;
    section_id: number;
}
...
export interface PartPopularityScore {
    part_num: string;
    score: number;
...
export interface LoosePartsInventory {
    id: string; // Unique identifier for this loose parts collection
    name: string; // User-defined name for this collection
    description?: string;
    parts: Record<string, LoosePartEntry>; // part_num -> LoosePartEntry
    lastUpdated: number;
    sortPreferences?: {
        field: 'partName' | 'partNumber' | 'category' | 'quantity' | 'lastUpdated' | 'popularity';
        direction: 'asc' | 'desc';
    };
    viewPreferences?: {
